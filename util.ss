...
(module util mzscheme
  ...
  (define (longest-common-sublabel label-1 label-2)
    (let ((label-1-marks (make-hash-table))
          (label-2-marks (make-hash-table))
          (deepest-node #f)
          (deepest-depth 0))
      (letrec
          [
           ...
           (mark-with-label-1!
            (lambda (node)
              (hash-table-put! label-1-marks node #t)))

           (mark-with-label-2!
            (lambda (node)
              (hash-table-put! label-2-marks node #t)))

           (marked-by-label-1?
            (lambda (node)
              (hash-table-get label-1-marks node false-thunk)))

           (marked-by-label-2?
            (lambda (node)
              (hash-table-get label-2-marks node false-thunk)))
